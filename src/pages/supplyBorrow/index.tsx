...
//
//     const fiatEquivalent = selectedToken && assetValue !== null
...
//                                     {mode === "supply" ? `${selectedToken.apy}%` : `${selectedToken.apr}%`}
//                                 </p>
//                             </div>
//
//                             <div className="mt-4">
...
const SupplyBorrow = () => {
    ...
    const queryMode = searchParams.get("mode") as "supply" | "borrow";
    ...
    const [mode, setMode] = useState<"supply" | "borrow">(queryMode || "supply");
    const [selectedToken, setSelectedToken] = useState<TokenData | null>(null);
    const [assetValue, setAssetValue] = useState<number | null>(null);
    const [selectedPercentage, setSelectedPercentage] = useState<number | null>(null);
    const [dropdownOpen, setDropdownOpen] = useState(false);
    ...
    useEffect(() => {
        if (tokenData && tokenData.length > 0) {
            const tokenMatch = tokenData.find(
                (t) =>
                    t.symbol.toLowerCase() === tokenSymbolFromQuery?.toLowerCase() &&
                    t.name.toLowerCase() === tokenNameFromQuery?.toLowerCase()
            );
            setSelectedToken(tokenMatch || tokenData[0]);
        }
    }, [tokenData, tokenSymbolFromQuery, tokenNameFromQuery]);
